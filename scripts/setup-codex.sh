...
MANIFEST="codex-workbench.services.json"

cd "$REPO_ROOT"

echo "Exporting Codex MCP config from $MANIFEST"
node --experimental-transform-types packages/launcher/src/cli.ts export --manifest "$MANIFEST"
...
echo "  - use codex-marketplace/ as a local folder source"
echo "  - or use this repository as the source and set Sparse paths to codex-marketplace"
...
echo "To remove this managed setup later, run ./scripts/uninstall-codex.sh"

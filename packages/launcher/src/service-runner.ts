...
function classifyBurpFailure(url: string, reason: string, stderr: string): string {
  if (
    ...
    return [
      ...
      `4. Leave the listener on ${url} or update plugins/codex/services.json to match Burp.`,
      '5. Rerun the launcher.',
      'If you do not want Burp for this run, disable the burp service in plugins/codex/services.json or run a subset of services.',
      `Proxy detail: ${reason}`
    ].join(' ');
  }
  ...
  if (reason.includes('HTTP 404') || reason.includes('HTTP 405')) {
    return [
      ...
      'If you do not want Burp for this run, disable the burp service in plugins/codex/services.json.',
      `Probe detail: ${reason}`
    ].join(' ');
  }

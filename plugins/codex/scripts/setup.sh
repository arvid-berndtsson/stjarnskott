...
MANIFEST="plugins/codex/services.json"

cd "$REPO_ROOT"

echo "Exporting Codex MCP config from $MANIFEST"
node --experimental-transform-types plugins/codex/src/cli.ts export --manifest "$MANIFEST"

echo
echo "Installing managed MCP block into your Codex config"
node --experimental-transform-types plugins/codex/src/cli.ts install
...
echo "  - use plugins/codex/marketplace/ as a local folder source"
echo "  - or use this repository as the source and set Sparse paths to plugins/codex/marketplace"
...
echo "To remove this managed setup later, run ./plugins/codex/scripts/uninstall.sh"

...
The current Burp MCP module assumes that `mcp-server/libs/mcp-proxy-all.jar` already exists before builds, tests, or local proxy launches can succeed. This is fragile because the proxy artifact is treated as manually managed workspace state rather than as a declared build input.
...
- Remove the manual dependency on `mcp-server/libs/mcp-proxy-all.jar`.
...
3. `run-burp-mcp-proxy.sh` depends on a manually placed file under `mcp-server/libs/`.
...
Use an included Gradle build or sibling workspace module for the proxy source, and have `mcp-server` consume the proxy shaded JAR as a declared build artifact.
...
- `mcp-server` resolves that artifact through Gradle task wiring.
...
No tracked file in `mcp-server/libs/` is treated as required mutable setup state.

### Artifact Flow

1. Gradle builds the proxy shaded JAR from source.
2. `mcp-server` copies or resolves that JAR into a generated location under `build/`.
...
`mcp-server/build.gradle.kts` should be updated so that:
...
`run-burp-mcp-proxy.sh` should stop assuming `mcp-server/libs/mcp-proxy-all.jar` exists.
...
- developers can launch the proxy locally without maintaining `mcp-server/libs/` by hand

...
If you want to use the Stdio proxy server directly from this repository, run `./gradlew syncProxyJar` in `integrations/burp/` first so `build/generated/proxy/mcp-proxy-all.jar` is available. You can also use the extension's installer option to extract the proxy server jar from Burp.

...
test('Burp and Vanta plugins expose separate Codex plugin bundles', async () => {
  const burpPlugin = JSON.parse(
    await readFile(path.resolve('plugins/codex/marketplace/plugins/burp/.codex-plugin/plugin.json'), 'utf8')
  ) as PluginConfig;
  const vantaPlugin = JSON.parse(
    await readFile(path.resolve('plugins/codex/marketplace/plugins/vanta/.codex-plugin/plugin.json'), 'utf8')
  ) as PluginConfig;
...
test('split manifest files isolate Burp and Vanta services', async () => {
  const burpManifest = JSON.parse(
    await readFile(path.resolve('plugins/codex/services.burp.json'), 'utf8')
  ) as { services: Array<{ id: string }> };
  const vantaManifest = JSON.parse(
    await readFile(path.resolve('plugins/codex/services.vanta.json'), 'utf8')
  ) as { services: Array<{ id: string }> };
...
test('repo marketplace exposes separate Stjarnskott plugin install choices', async () => {
  const marketplace = JSON.parse(
    await readFile(path.resolve('plugins/codex/marketplace/.agents/plugins/marketplace.json'), 'utf8')
  ) as {
    name: string;
